...
<template>
                    ...
                    <div class="flex flex-col gap-2">
                        <div class="flex flex-row justify-between items-start gap-4">
                            ...
                            <div class="flex flex-col justify-start w-2/3">
                                ...
                                <p class="text-2xl font-bold text-gray-800 w-full">
                                    {{ pet.name }}
                                </p>
                            </div>
                        </div>
                        ...
                        <p class="flex flex-row justify-between">
                            <strong>Sünnikuupäev:</strong> {{ formatDate(pet?.dob) }}
                        </p>
                        <p class="flex flex-row justify-between">
            ...
            <section class="bg-[#FFFDF5] p-6 rounded-2xl w-full max-w-md mx-auto">
                <div class="flex flex-col gap-6">
                    <div class="flex flex-row justify-between items-center">
                        ...
                        <button @click="showMedicalModal = true" class="text-green-700 font-medium flex justify-center items-center border border-green-700 rounded-full hover:bg-green-700 hover:text-white transition w-5 h-5 leading-none">+</button>
                    </div>

                    <div class="flex flex-col gap-3">
                        <h4 class="text-sm font-medium text-gray-700">Vaktsiinid</h4>
                        <p v-if="!pet.vaccines || pet.vaccines.length === 0" class="text-sm text-gray-400 text-center py-4">Vaktsiine pole veel lisatud.</p>
                    ...
                    <div class="flex flex-col gap-3 mt-4">
                        ...
                        <p v-if="!pet.medications || pet.medications.length === 0" class="text-sm text-gray-400 text-center py-4">Ravimeid pole veel lisatud.</p>
            ...
            <section class="bg-[#FFFDF5] p-6 rounded-2xl w-full max-w-md mx-auto">
                <div class="flex flex-col gap-6">
                    <div class="flex flex-row justify-between items-center">
                        ...
                        <Link href="/pets/create" class="text-green-700 font-medium flex justify-center items-center border border-green-700 rounded-full hover:bg-green-700 hover:text-white transition w-5 h-5 leading-none">+</Link>
                    </div>
            ...
            <section class="bg-[#FFFDF5] p-6 rounded-2xl w-full max-w-md mx-auto">
                <div class="flex flex-col gap-6">
                    <div class="flex flex-row justify-between items-center">
                        ...
                        <button @click="showVetModal = true" class="text-green-700 font-medium flex justify-center items-center border border-green-700 rounded-full hover:bg-green-700 hover:text-white transition w-5 h-5 leading-none">+</button>
                    </div>

                    <div class="flex flex-col gap-3">
                        <p v-if="!pet.vet_visits || pet.vet_visits.length === 0" class="text-sm text-gray-400 text-center py-4">Arstivisiite pole veel lisatud.</p>
                        <div v-for="visit in pet.vet_visits" :key="visit.id" class="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
                            ...
                            <div v-if="expandedVisitId === visit.id" class="px-4 pb-4 border-t border-gray-100">
                                <p class="text-sm text-gray-600 whitespace-pre-wrap mt-3 leading-relaxed">{{ visit.log || "Logi puudub." }}</p>
                                <div v-if="visit.files && visit.files.length" class="mt-3 flex flex-col gap-2">
                                    ...
                                    <a v-for="file in visit.files" :key="file.id" :href="route('vet-visit-files.download', file.id)" class="flex items-center gap-2 bg-gray-50 hover:bg-gray-100 rounded-lg px-3 py-2 transition">
                                        <span class="text-red-400">📄</span>
                                        <span class="text-xs text-gray-700 truncate">{{ file.original_name }}</span>
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-gray-400 ml-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                ...
                                <div class="mt-3">
                                    <label class="flex items-center justify-center gap-2 border-2 border-dashed border-gray-200 rounded-xl py-3 cursor-pointer hover:border-[#2D5A3D] transition">
                                        ...
                                        <input type="file" multiple accept=".pdf" class="hidden" @change="(e) => uploadFiles(e, visit)" />
                                    </label>
                                    <div v-if="pendingFiles[visit.id] && pendingFiles[visit.id].length" class="mt-2 flex flex-col gap-1">
                                        ...
                                        <button @click="submitFiles(visit)" class="mt-1 w-full text-xs bg-[#275342] text-white py-2 rounded-lg hover:bg-[#1e3f31] transition">Lae üles</button>
                                    </div>
                                </div>
                                <div class="flex justify-end mt-3">
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
        <div class="bg-white rounded-2xl p-6 w-80 shadow-xl flex flex-col gap-4">
            <h2 class="text-lg font-semibold text-gray-800 text-center">Kustuta lemmik</h2>
            <p class="text-sm text-gray-500 text-center">
                Kas oled kindel, et soovid <strong>{{ pet.name }}</strong> kustutada? Seda tegevust ei saa tagasi võtta.
            </p>
